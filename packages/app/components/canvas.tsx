...
// A unique identifier used to identify local operations
const local = Math.random().toString();
// global Yjs document and array references
...
export default function Canvas() {
  ...
  const onReady = async (e: CustomEvent<any>) => {
    ...
    // Initialize Yjs documentation and IndexedDB persistence
    if (!doc) {
      doc = new Y.Doc();
      ...
      // Set up the API's onchange callback to synchronize canvas changes to Yjs
    ...
    api.runAtNextTick(() => {
      // Update only when there are no saved nodes (avoid overwriting data loaded from IndexedDB)
      if (savedNodes.length === 0) {
        api.updateNodes(nodes);
        api.selectNodes([nodes[0]]);
      } else {
        // If there are saved nodes, make sure they are loaded correctly
