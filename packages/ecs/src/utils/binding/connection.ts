...
export function getFixedTerminalPoint(edge: SerializedNode, terminal: SerializedNode, source: boolean, constraint: ConstraintAttributes) {
  ...
  // Step 1: If there is a constraint, calculate the actual connection point via getConnectionPoint
  if (!isNil(constraint)) {
    pt = getConnectionPoint(terminal, constraint);
  }

  // Step 2: If there is no end node (dangling edge), get the endpoint from the geometry of the edge
...
export function getConnectionPoint(vertex: SerializedNode | null, constraint: ConstraintAttributes): IPointData | null {
  ...
  // Step 1: If constraint point information is available, calculate the constraint-based connection point
  if (!isNil(vertex) && !isNil(constraint.x) && !isNil(constraint.y)) {
    ...
    // Step 2: Calculate the actual point position of the relative coordinates
    // constraint.x/y Yes normalized coordinates (0-1 range)
    // constraint.dx, constraint.dy It's a pixel shift
    point = {
