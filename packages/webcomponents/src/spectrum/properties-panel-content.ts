...
export class PropertiesPanelContent extends LitElement {
  ...
  private handlePaddingSideChanged(index: 0 | 1 | 2 | 3, e: Event) {
    ...
    const sides = normalizeBoxSides((this.node as FlexNode).padding) as [
      number,
      number,
      number,
      number,
  ...
  private handleMarginSideChanged(index: 0 | 1 | 2 | 3, e: Event) {
    const raw = PropertiesPanelContent.parseNumberFieldValue(e);
    const next = raw !== undefined && Number.isFinite(raw) ? raw : 0;
    const sides = normalizeBoxSides((this.node as FlexNode).margin) as [
      number,
      number,
      number,
      number,
  ...
  private handleLayoutColumnGapChanged(
  ...
  private handleFlexBasisChanged(e: Event) {
    const t = e.target as HTMLElement & { value?: number | string };
    const raw =
      t.value === undefined || t.value === '' ? '' : String(t.value).trim();
    if (raw === '') {
      this.api.updateNode(this.node, {
        flexBasis: undefined,
      } as Partial<SerializedNode>);
    ...
    return html`
      ...
      <div class="line">
        ...
        <sp-number-field
          id=${`fi-minw-${safeId}`}
          size="s"
          .value=${minW !== undefined && Number.isFinite(minW)
            ? minW
            : undefined}
      ...
      <div class="line">
        ...
        <sp-number-field
          id=${`fi-maxw-${safeId}`}
          size="s"
          .value=${maxW !== undefined && Number.isFinite(maxW)
            ? maxW
            : undefined}
          placeholder=${msg(str`Auto`)}
      ...
      <div class="line">
        ...
        <sp-number-field
          id=${`fi-minh-${safeId}`}
          size="s"
          .value=${minH !== undefined && Number.isFinite(minH)
            ? minH
            : undefined}
          placeholder=${msg(str`Auto`)}
      ...
      <div class="line">
        ...
        <sp-number-field
          id=${`fi-maxh-${safeId}`}
          size="s"
          .value=${maxH !== undefined && Number.isFinite(maxH)
            ? maxH
            : undefined}
          placeholder=${msg(str`Auto`)}
      ...
      <div class="content layout-group style-group">
        <div class="line">
          ...
          >
            ...
            <sp-menu-item value="flex-start">${msg(str`Start`)}</sp-menu-item>
  ...
  private layoutTemplate() {
    ...
    const paddingSides = normalizeBoxSides(n.padding);
    const paddingUniform = boxSidesUniform(paddingSides);
    const padTriggerId = `ic-pad-trg-${this.node.id.replace(
      /[^a-zA-Z0-9_-]/g,
      '_',
    )}`;
    const marginSides = normalizeBoxSides(n.margin);
    const marginUniform = boxSidesUniform(marginSides);
    const marTriggerId = `ic-mar-trg-${this.node.id.replace(
      /[^a-zA-Z0-9_-]/g,
      '_',
    )}`;
    const gap = n.gap ?? 0;
      ...
      <div class="content layout-group style-group">
        ...
        <div class="line">
          ...
          <sp-picker
            size="s"
            .value=${flexWrap}
            @change=${this.handleFlexWrapChanged}
          >
  ...
  render() {
    ...
    return html`
      <sp-accordion allow-multiple size="s">
              ...
              >
                <div class="content style-group">
                  ...
                  ${when(
                    ...
                    () => html`
                      <div class="line">
                        <sp-field-label for="corner-radius" side-aligned="start"
        ...
        ${when(this.node.display === 'flex', () => this.layoutTemplate())}
