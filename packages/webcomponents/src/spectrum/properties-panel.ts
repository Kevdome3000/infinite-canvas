...
export class PropertiesPanel extends LitElement {
  ...
  private get multiSelectSectionsOpenResolved(): {
    ...
    const s = this.appState
      .propertiesPanelSectionsOpen as AppState['propertiesPanelSectionsOpen'] & {
  ...
  private clampPanelWidth(w: number): number {
    const max = Math.min(
      MAX_PANEL_WIDTH_PX,
      typeof window !== 'undefined'
        ? Math.max(MIN_PANEL_WIDTH_PX, Math.floor(window.innerWidth - 80))
        : MAX_PANEL_WIDTH_PX,
    );
  ...
  private renderPanelRoot(inner: TemplateResult) {
    return html`
      ...
      >
        ...
        <div class="panel-column">${inner} ${this.renderResizeHandle()}</div>
      </div>
    `;
  }
      ...
      return this.renderPanelRoot(html`<section>
        ...
        >
          <p class="multi-select-hint">
            ${layersSelected.length} ${msg(str` selected`)}
          </p>
          <sp-accordion class="multi-select-accordion" allow-multiple size="s">
            <sp-accordion-item
              ...
              <div class="content">
                ...
                >
                  ...
                  <sp-action-button
                    quiet
                    size="s"
                    @click=${() => this.handleDistributeSpacing('horizontal')}
                  ...
                  <sp-action-button
                    quiet
                    size="s"
                    @click=${() => this.handleDistributeSpacing('vertical')}
